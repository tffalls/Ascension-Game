...
//  FallsAssignment9
//
//  Created by Tarryn Falls on 11/25/17.
...
class Game2: SKScene, SKPhysicsContactDelegate {
    ...
    func makeItRain() {
        rainTimer = Timer.scheduledTimer(withTimeInterval: 8.0, repeats: true, block: { (timer) in
    ...
    func createBackdrop() {
        ...
        var w: CGFloat = 336.0
        var x: CGFloat = w / 2.0
        let backTexture = SKTexture(imageNamed: "Jungle-Desktop-Wallpapers.png")
    ...
    func createSceneObjects() {
        ...
        scoreNode.fontColor = NSColor.black
        scoreNode.text = "Start Climbing!"
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if contact.bodyA.node?.name == "ceiling" && contact.bodyB.node?.name == "Monkey" {
            ...
            view?.presentScene(controller.game3, transition: doors)
